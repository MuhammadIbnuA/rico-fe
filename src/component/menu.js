...
function App() {
  ...
  return (
    <div>
      <h1>Create Data Pejabat</h1>
      <form onSubmit={handleSubmit}>
        <input type="text" value={nama} onChange={(e) => setNama(e.target.value)} placeholder="Nama" />
        <input type="text" value={harga} onChange={(e) => setHarga(e.target.value)} placeholder="Alamat" />
        <input type="text" value={kategori} onChange={(e) => setKategori(e.target.value)} placeholder="Jabatan" />
        <button type="submit">{selectedMenuId ? "Update Data" : "Create Data"}</button>
      </form>
...
async function createMenu(menuData) {
  try {
    ...
    const response = await fetch("https://rico-be.vercel.app/menu", {
      method: "POST",
      headers: {
...
async function updateMenu(menuId, updateData) {
  try {
    const token = localStorage.getItem("token");
    const response = await fetch(`https://rico-be.vercel.app/menu/${menuId}`, {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${token}`,
